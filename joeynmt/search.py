...
def fcfs_beam_search(model: Model, beam_size: int,
    ...
    for step in range(max_output_length):
        ...
        # For Transformer, we feed the complete predicted sentence so far.
        # For Recurrent models, only feed the previous target word prediction
        decoder_input = alive_seq  # complete prediction so far. shape: (batch_size * beam_size, step+1)
        ...
        # compute log probability over trg vocab
        log_probs = F.log_softmax(logits, dim=-1).squeeze(1)  # (remaining_batch_size * beam_size, trg_vocab_size)

        # multiply probs by the beam probability (= add log_probs)
        log_probs += topk_log_probs.view(-1).unsqueeze(1)  # (remaining_batch_size * beam_size, trg_vocab_size)
        curr_scores = log_probs.clone()
        ...
        # flatten log_probs into a list of possibilities and get remaining_bath_size
        curr_scores = curr_scores.reshape(-1, beam_size * trg_vocab_size)  # (remaining_batch_size, beam_size, trg_vocab_size)
        remaining_batch_size = curr_scores.size(0)
        ...
        )  # (remaining_batch_size, beam_size_)
        ...
        for b in range(remaining_batch_size):
            ...
            if end_condition[b]:
                ...
                for n, (score, pred) in enumerate(best_hyp):
                    ...
                    results["predictions"][b_org].append(pred)
